...
void vSSA::insertSigmas(TerminatorInst *TI, Value *V) {
  ...
  for (unsigned i = 0, e = TI->getNumSuccessors(); i < e; ++i) {
    ...
    if (BB_next != BB && BB_next->getSinglePredecessor() != NULL &&
        dominateOrHasInFrontier(BB, BB_next, V)) {
      // Create the sigma function (but before, verify if there is already an
      // identical sigma function)
      ...
      // If it was the first sigma created for V, phis may be needed, so we
      // verify and create them
      if (firstSigma) {
        // Insert vSSA_phi, if necessary
        vssaphi_created = insertPhisForSigma(V, sigma);
      } else {
        // If it was not the first sigma created for V, then the phis already
        // have been created and are stored in the deque, so we need only to
        // insert the sigma as operand of them
        insertSigmaAsOperandOfPhis(vssaphi_created, sigma);
      }
    }

    // If phis were created, no more phis will be created, only operands will be
    // inserted in them
 ...
 *   - All uses of V in the dominator tree of sigma(V) are renamed, except for
 * the sigma itself, of course
...
void vSSA::renameUsesToSigma(Value *V, PHINode *sigma) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    else if ((DF_BB != DF_->end()) &&
             (DF_BB->second.find(BB_user) != DF_BB->second.end())) {
...
SmallVector<PHINode *, 25> vSSA::insertPhisForSigma(Value *V, PHINode *sigma) {
  ...
  for (std::set<BasicBlock *>::iterator DFit = DF_BB->second.begin(),
                                        DFend = DF_BB->second.end();
       DFit != DFend; ++DFit) {
    ...
    // If the original Value, for which the sigma was created, dominates the
...
void vSSA::insertPhisForPhi(Value *V, PHINode *phi) {
  ...
  for (std::set<BasicBlock *>::iterator DFit = DF_BB->second.begin(),
    ...
    // If the original Value, for which the phi was created, dominates the
    // basicblock in the frontier, and this BB dominates any use of the Value, a
    // vSSA_PHI is needed
    if (condition) {
 ...
 *   - Uses of V in the same basicblock of vSSA_PHI are only renamed if they are
 * not in PHI functions
...
void vSSA::renameUsesToPhi(Value *V, PHINode *phi) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    else if (DF_BB->second.find(usepointers[i]->getParent()) !=
             DF_BB->second.end()) {
      // Check if the user is a PHI node (it has to be, but only for precaution)
  ...
  // Creation of phis may require the creation of sigmas that were not created
  // previosly, so we do it now
...
void vSSA::insertSigmaAsOperandOfPhis(
  ...
  for (SmallVectorImpl<PHINode *>::iterator vit = vssaphi_created.begin(),
    ...
    for (; PI != PE; ++PI) {
      ...
      if (DT_->dominates(
              BB, predBB) /* && (vssaphi->getBasicBlockIndex(predBB) == -1)*/) {
...
/*
 *  When phis are created, only the sigma and phi operands are inserted into
 * them. Thus, we need to insert V, for which sigmas and phis were created, as
 * incoming value of all
...
void vSSA::populatePhis(SmallVector<PHINode *, 25> &vssaphi_created, Value *V) {
  ...
  for (SmallVectorImpl<PHINode *>::iterator vit = vssaphi_created.begin(),
    ...
    // If a predecessor already has incoming values in the vSSA_phi, we don't
    // count them
...
bool vSSA::dominateAny(BasicBlock *BB, Value *value) {
  ...
  // If the use is in the BasicBlock in-frontier itself and it's a PHINode, it
  // means that mem2reg already created a phi for this case, so we don't need to
  // do it
  // Otherwise, if the use is dominated by the BasicBlock in-frontier, we create
  // the vSSA_PHI
...
/*
 *  Used to insert sigma functions. Verifies if BB_next dominates any use of
 * value (except if the use is in the same basicblock in a phi node)
...
bool vSSA::dominateOrHasInFrontier(BasicBlock *BB, BasicBlock *BB_next,
  ...
  for (Value::user_iterator begin = value->user_begin(),
    ...
    // create a sigma in BB_next
    // to split the lifetime of variables
    if ((BB_father != BB) && (DF_BB != DF_->end()) &&
        (DF_BB->second.find(BB_father) != DF_BB->second.end()))
      return true;
  }
  return false;
}
...
bool vSSA::verifySigmaExistance(Value *V, BasicBlock *BB, BasicBlock *BB_from) {
  for (BasicBlock::iterator it = BB->begin(); isa<PHINode>(it); ++it) {
    ...
    if ((sigma->getIncomingValue(0) == V) &&
        (sigma->getIncomingBlock(0) == BB_from))
      return true;
  }
